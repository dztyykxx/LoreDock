...
为项目知识整理提供可追溯、可多轮继续的长期任务会话，使单一 Agent 在服务端固定范围内通过版本化 Tool 新增或修改多篇工作文档，以原生 AssistantMessage 公开有限决策，并确保正式知识始终由管理员基于完整修订集合和服务端 Diff 原子发布。
...
系统 SHALL 只允许管理员对仍为 PROCESSING 的任务执行关闭或放弃。确认无需变更 MUST 要求工作区没有有效工作文档和非空有限结论，并把任务设为 CLOSED_NO_CHANGE、候选输入设为 CURATED、在同一事务内归档候选输入草稿文档使其退出待处理草稿池（与发布吸收候选输入的行为一致，已归档草稿不得再次被选入新任务）。放弃 SHALL 把任务设为 ABANDONED、候选输入恢复 PENDING，候选草稿保持 DRAFT 留在待处理草稿池等待重新整理。两种操作 SHALL 保留全部历史并使任务只读。

#### Scenario: 确认无需变更
- **WHEN** 工作区没有有效文档且管理员提交合法结论
- **THEN** 系统原子关闭任务、标记候选输入已整理、归档候选输入草稿文档使其退出待处理草稿池并写入系统记录

#### Scenario: 有工作文档时确认无需变更
- **WHEN** 当前存在至少一份有效工作文档
- **THEN** 服务端拒绝关闭且不改变任务或候选输入状态，候选草稿不被归档

#### Scenario: 放弃任务
- **WHEN** 管理员放弃 PROCESSING 任务
- **THEN** 系统原子保留历史、恢复候选输入为 PENDING、把任务标记 ABANDONED并拒绝后续消息，候选草稿保持 DRAFT 留在待处理草稿池
