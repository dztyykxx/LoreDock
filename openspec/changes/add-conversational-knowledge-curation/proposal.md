...
本变更依据产品需求 v1.3 的 `FR-KG-11～15`、`FR-AGENT-17～20` 和开发计划 T6B/T8，把知识任务定义为系统或管理员触发的长期会话，并把草稿定义为通过安全 Tool 增量修改、按修订和 Diff 审批的独立产物。
...
- 非目标：不建设逐 hunk 合并器、Git 式草稿分支、多人实时协同编辑、模型流中途续传或任意脚本式文件修改。
...
- 后端：知识任务会话/消息、Agent run 关联、暂停状态机、草稿修订、来源关联、Diff 查询和发布修订锁定。
- Agent 平台：新增 `draft_create`、`draft_read`、`draft_update`、`draft_diff` Tool，并接入 PostgreSQL Checkpoint 和稳定 threadId。
