...
- [x] 8.1 全量后端单元测试、代表性 PostgreSQL 集成测试与前端测试通过；唯一 Flyway 基线可空库启动。
- [x] 8.2 人工审查所有模块 diff：无规格外新功能、无未来扩展点、无新增低价值类与表。
- [x] 8.3 同步 `backend-module-architecture` 增量到主规格，更新必要文档，严格校验并归档本 Change。
