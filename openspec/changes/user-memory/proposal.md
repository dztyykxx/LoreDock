...
- 知识整理会话主 Agent（会话图顶层 `main_agent`）：
  ...
  - 新业务工具仅注册给主 Agent：`memory_search`（摘要级搜索）、`memory_read`（按需加载全文并计入使用频次）、`memory_write`（提炼写入，内嵌新增/重复/增量/冲突判断，增量原地合并，未决冲突转人工确认）；
...
- `user-memory`: 用户记忆的数据模型与生命周期（范围/分类/状态/溯源/频次）、REST 读写管理、关键词+过滤+频次的确定性检索打分、按需全文加载、写入时的新增/重复/增量/冲突判断、版本历史与预算约束。
