...
describe('router session guards', () => {
  ...
  it('declares global and project knowledge routes without replacing settings', () => {
    ...
    expect(router.resolve('/projects/network-designer/knowledge-tasks').name).toBe('project-knowledge-tasks')
    expect(router.resolve('/projects/network-designer/knowledge-tasks/41').name).toBe('project-knowledge-task')
    expect(router.getRoutes().map(route => route.name)).not.toContain('project-code-snapshots')
