      ...
      <section class="knowledge-content">
        ...
        <div v-else class="knowledge-workspace">
          <aside class="knowledge-directory-panel">
            <div class="knowledge-panel-heading"><h2>{{ isDraftWorkspace ? '草稿目录' : '文档目录' }}</h2><span>{{ totalElements }}</span></div>
          ...
          <section class="knowledge-list-panel">
            <div class="knowledge-list-toolbar">
              <div><h2>{{ currentDirectory || (isDraftWorkspace ? '全部草稿' : '全部文档') }}</h2><p>{{ isDraftWorkspace ? '仅待处理草稿' : isAdministrator ? '全部生命周期状态' : '仅已发布文档' }}</p></div>
              <div class="knowledge-list-toolbar__actions">
                <label v-if="isDraftWorkspace && draftsOnPage.length" class="knowledge-select-all">
