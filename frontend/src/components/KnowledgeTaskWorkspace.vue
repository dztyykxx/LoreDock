  ...
  <section class="knowledge-task-workspace">
    <div data-testid="knowledge-task-conversation" class="knowledge-task-panel knowledge-task-conversation">
      ...
      <ol class="message-list">
        <li v-for="message in task.messages" :key="message.messageId">
...
interface Task { goal: string; currentDraftRevision?: number | null; messages: Message[]; runs: Run[]; events?: TaskEvent[] }
...
function messageLabel(role: string): string {
  return ({ SYSTEM_TRIGGER: '系统触发', USER: '你', COORDINATOR_AGENT: '知识整理 Agent', SUB_AGENT: '复核 Agent', TOOL: '工具结果' } as Record<string, string>)[role] ?? role
}
