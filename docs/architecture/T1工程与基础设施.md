...
│   └── model          request/response/command/result/entity/enums
...
Controller 只负责协议解析、认证上下文和响应映射；Service 集中业务规则与事务；Mapper 只保存 MyBatis 接口；所有实体、请求、响应、命令、结果和快照进入模块自己的 `model` 子包。Service 可以直接访问本模块 Mapper，跨模块只能调用对方 Service，禁止跨模块 Mapper 访问。只为标准 `ChatModel`、`EmbeddingModel`、Agent Runtime、Agent 定义和对象存储等真实替换边界保留接口。
