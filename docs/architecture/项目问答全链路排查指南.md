...
sequenceDiagram
    ...
    participant AGENT as ProjectQaAgentExecutor
    ...
    RUN->>DB: RUN_STARTED
    EXEC->>AGENT: 执行具体项目问答 Agent
    AGENT->>MODEL: classpath Skill + 范围 + 输出 Schema
...
| `AGENT_SKILL_UNAVAILABLE` | classpath 中 `project-qa` Skill 和输出 Schema |
...
- `backend/src/main/java/io/github/loredock/agent/service/impl/ProjectQaAgentExecutor.java`
...
具体执行器直接使用 Spring AI Alibaba 的 `ReactAgent`、`ClasspathSkillRegistry`、`SkillsAgentHook`、调用限制 Hook 和 Interceptor；没有额外的通用 Runtime、Skill Provider 或 Front Matter Parser。业务代码只保留范围固定、上下文裁剪、证据台账、可信引用和拒答规则。

当前 `project-qa` 只注册一个只读工具：

| 工具 | 范围约束 |
|---|---|
| `knowledge_search` | 固定项目、分支和活动知识 generation |

每次工具调用必须复核运行仍为 `RUNNING`，并检查活动知识 generation 没有变化。结果经过相关度、数量、片段长度和总上下文上限裁剪后登记到 `agent_evidence`。工具结果正文只进入本次有界模型上下文，不进入事件或证据表。
...
2. `ANSWER` 至少引用一条证据。
3. `BUSINESS_RULE` 只允许引用知识证据。
4. 未调用工具的普通对话可使用 `answerBasis=null`、`citations=[]`，但不能承载项目事实。
5. 要求确认当前代码实现时必须拒答或提示核对本地代码，不能把文档当成当前实现。
