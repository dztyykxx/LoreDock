...
3. 数据库不保存 token、回答分片或拒答正文事件。最终正文以 `agent_run.result_text` 和唯一 `ASSISTANT` 消息为准。
...
sequenceDiagram
    ...
    TOOL->>DB: 保存有限 evidence + SOURCE_FOUND
    ...
    DB-->>SSE: 事务提交后通知
    SSE-->>FE: 粗粒度事件
    FE->>HTTP: 收到终态后 GET 详情
    HTTP-->>FE: 正文 + citations
```
...
`agent_run` 只保存问题哈希和 Unicode 长度；问题原文保存到 `web_qa_message/USER`。运行受理与 `RUN_ACCEPTED` 在同一短事务中完成，外层问答事务提交后才调度执行。
...
事件 payload 只保存允许公开的状态、工具结果数量、结果类型或错误码，不保存问题、答案、拒答正文和证据正文。
...
前端收到 `run.completed`、`run.failed` 或 `run.terminated` 后关闭 SSE 并调用详情接口。最终页面以详情返回的 `resultText` 和 `citations` 为准，不依赖累计的正文事件。
...
sequence 应从 1 严格递增，不应出现 `ANSWER_DELTA`、`REFUSAL`、逐工具开始/结束事件或正文 payload。
