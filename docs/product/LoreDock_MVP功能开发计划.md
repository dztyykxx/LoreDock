...
| 当前进度 | T1、T2、T3 已完成；下一个可独立启动的任务为 T4 代码快照与全文索引 |
...
flowchart TD
    ...
    T3 --> T5["T5 知识混合检索"]
    T4 --> T5
    T5 --> T6["T6 可恢复多 Agent Runtime、Skill 与项目记忆"]
    T6 --> T7["T7 Web 项目问答"]
    T6 --> T8["T8 普通知识生成与审核"]
    T6 --> T9["T9 需求到实现知识沉淀"]
...
### [ ] T6：可恢复多 Agent Runtime、Skill 与项目记忆

目标：建立受控、可追溯、可从检查点恢复且不能越权发布知识的统一 Agent 执行环境，为单 Agent 问答和多 Agent 文档整理提供共同基础。

覆盖需求：FR-AGENT-01～16。

功能范围：

- 先验证并锁定 Spring Boot 3.5.x、Spring AI 1.1.2 和 Spring AI Alibaba 1.1.2.x 正式版本，使用 Spring AI Alibaba Graph 实现固定工作流；
- 定义模型与 Graph 运行适配接口，并提供 MiniMax 实现和测试用 Fake Model；
- 支持单 Agent 问答，以及证据并行提取、编写、独立审查、有限返工和人工暂停的固定多 Agent Graph；
- 加载并版本化 `project_qa`、`document_writer`、`change_documenter`、`knowledge_audit`、`knowledge_curator` 及需求、代码、测试证据和独立审查角色 Skill；
- 为每个项目提供可编辑、可版本化的 `PROJECT_MEMORY.md`，并在运行开始时固定 Skill、项目记忆、工作流和模型版本；
- 校验每个 Skill 的工具白名单和工具参数；
- 限制最大步骤数、一次审查返工、超时、检索数量、上下文长度和模型调用次数；
...
- 保存 Skill 和项目记忆版本、模型、工具调用摘要、输入来源、引用、Token、公开阶段事件和运行状态；
- 支持 Skill 启用、停用和版本更新；
- 支持证据不足或独立审查阻断时暂停，并由管理员补充方向、允许保留待核实项、要求重写或取消；
- 将写入能力限制为草稿、报告和知识缺口，禁止 Agent 发布正式知识。

重点验收：越权工具调用和跨项目读取被拒绝；达到步骤或时间限制后安全停止；失败运行保留成功中间产物且不产生半发布状态；修改 Skill 或项目记忆后新旧运行结果可区分；IDEA 停止并重启后端后可继续原运行且不重复已完成节点。
