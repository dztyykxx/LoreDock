...
| 当前进度 | T1～T7A、T9 与记忆模块（用户确认新增、超基线）已完成；T8 的代码与自动化测试已完成，待真实模型演示验收；剩余主线为 T8 演示验收与 T10 集成验收 |
...
- 当前阶段不建设同 run 恢复、项目长期记忆、数据库/Web Skill 市场、可视化编排器、运行时任意创建 Agent 或多实例自动抢占（用户偏好记忆已确认新增，见 3.10 记忆模块；这里的不建设语义指知识事实类记忆，不含偏好记忆）。
...
### [x] 记忆模块：用户长期记忆（超出 MVP 基线、用户确认新增）

目标：把用户在对话中明确表达的长期偏好沉淀为共享记忆，在知识整理主 Agent 上下文中稳定注入，使偏好跨会话自动生效；记忆只表达偏好，不作为知识事实或证据。

覆盖需求：非 MVP 需求基线功能；由用户确认新增（OpenSpec change `user-memory`，执行方式与其余任务一致：OpenSpec、接口优先与 TDD）。

功能范围：

- 单表 `user_memory`：共享语义不按用户隔离，GLOBAL/PROJECT 两级范围（会话挂项目为 PROJECT，否则 GLOBAL），分类与标题/摘要/正文限长，来源溯源（run、会话、类型）与单 run 写入预算；
- 主 Agent 上下文预载注入：检索范围 GLOBAL∪本项目、关键词匹配（无向量）+ 无命中高频兜底，摘要 ≤300 码点、最多 30 条、行尾带编号；记忆快照按 run 固定，同一次请求内注入不变；
- `memory_search/memory_read/memory_write` 三工具只注册主 Agent（专家白名单不含记忆）；`memory_write` 由服务端判断链裁决（值得写/语义重复跳过且停用不复活/冲突仍写双 ACTIVE/单 run 预算）
，范围由会话自身决定、工具内回查 run 固定范围；
- `GET /api/memories`（登录即读）+ `POST/PUT/DELETE /api/admin/memories`（仅 ADMIN）；Web、服务层与 Agent 复用同一业务逻辑；人工路径可管理停用但不得绕过 scope 校验；
- 记忆不得作为证据或来源引用、不进入专家视图，偏好经主 Agent 指令传达给专家；检索异常与超限均服务端强制，不阻塞执行。

重点验收：E2E 已验证「偏好沉淀 → 下一轮【用户记忆】块注入（run 内两次主 Agent 注入相同）→ 起草指令携带偏好且记忆全文/编号不出现于专家视图，公开事件与人工发布门禁不变」；范围隔离、确定性打分、兜底、越权拒答、判断链、预算与 REST 权限错误语义分别由单元/集成用例验证；OpenSpec change `user-memory` 全部任务勾选完成。

### [ ] 记忆合并与历史查看（设计待审阅，待实施）

前置依赖：已交付 memory 模块、记忆管理页，以及记忆预载显式字段映射修复。实施依据：[用户记忆合并实现设计](../architecture/用户记忆合并实现设计.md)。

范围：处理新增、重复、增量和冲突；原 ID 合并更新、完整正文判断、明确更正证据、停用不复活、版本历史、短事务并发校验与修改预算，并在现有页面提供历史查看。具体处置规则按设计稿实施；当前仍是设计交付，不代表业务实现完成。

验收重点：真实汇报格式候选更新原记忆而不丢约束；再次重复不更新；明确改口与未决冲突分别处置；下一 run 注入新摘要；真实 PostgreSQL 和模型验证提供实际证据。

### [~] 上下文压缩轻量评估（用户确认的测试专项）

前置依赖：现有上下文组装/压缩模块与 Atlas 离线 Judge；按 [上下文压缩轻量评估方案](../quality/上下文压缩轻量评估方案.md) 实施。范围为六条长会话、完整/压缩历史配对续做、独立 LLM Judge 和两项指标报告，代码位于测试目录，不新增生产接口或数据库表。先验证真实压缩路径，再执行真实模型评估；状态不代表滚动摘要或完整端到端已验收。
