...
public class KnowledgeDocumentViewFactory {
    ...
    private final KnowledgeIndexSyncStateReader indexState;

    /** @param indexState 活动 generation 修订批量读取端口 */
    @Autowired
    public KnowledgeDocumentViewFactory(KnowledgeIndexSyncStateReader indexState) {
        this.indexState = indexState;
    }

    /** 仅供不启动 Spring 的既有领域测试使用，语义等同当前尚无活动 generation。 */
    public KnowledgeDocumentViewFactory() {
        this(documentIds -> new ActiveKnowledgeIndexRevisions(false, Map.of()));
    }
    ...
    public List<KnowledgeDocumentSummary> summaries(List<KnowledgeDocument> documents) {
        Map<java.util.UUID, KnowledgeIndexSyncStatus> statuses = syncStatuses(documents);
    ...
    private Map<java.util.UUID, KnowledgeIndexSyncStatus> syncStatuses(List<KnowledgeDocument> documents) {
        List<java.util.UUID> publishedIds = documents.stream()
                .filter(document -> document.status() == DocumentStatus.PUBLISHED)
                .map(KnowledgeDocument::id).toList();
        ActiveKnowledgeIndexRevisions active = indexState.readActiveRevisions(publishedIds);
        java.util.LinkedHashMap<java.util.UUID, KnowledgeIndexSyncStatus> statuses = new java.util.LinkedHashMap<>();
