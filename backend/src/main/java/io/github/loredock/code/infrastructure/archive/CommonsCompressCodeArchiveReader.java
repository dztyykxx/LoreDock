...
import io.github.loredock.code.application.CodeArchiveEntryConsumer;
import io.github.loredock.code.application.CodeArchiveReadPort;
...
/**
 * Commons Compress 中央目录适配器。对象只复制到服务端 UUID 工作目录的 {@code input.zip}，
 * 条目名称永远不参与磁盘路径；所有条目先完成结构校验，随后才允许流式读取正文。
 */
@Component
public class CommonsCompressCodeArchiveReader implements CodeArchiveReadPort {
    ...
    @Override
    public void read(UUID jobId, String objectKey, CodeArchiveEntryConsumer consumer) {
    ...
    private Path prepareJobDirectory(UUID jobId) throws IOException {
    ...
    private void readCheckedEntries(
            ...
            CodeArchiveEntryConsumer consumer
    ) throws IOException {
        Enumeration<ZipArchiveEntry> entries = zip.getEntries();
        int ordinal = 0;
        while (entries.hasMoreElements()) {
