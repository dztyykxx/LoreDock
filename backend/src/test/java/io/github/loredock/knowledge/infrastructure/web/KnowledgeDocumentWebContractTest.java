...
import io.github.loredock.knowledge.application.KnowledgeBrowseResult;
...
import io.github.loredock.knowledge.application.KnowledgeDirectoryNode;
import io.github.loredock.knowledge.application.KnowledgeDocumentNotFoundException;
import io.github.loredock.knowledge.application.KnowledgeDocumentQueryUseCase;
import io.github.loredock.knowledge.application.KnowledgeDocumentSummary;
import io.github.loredock.knowledge.application.KnowledgeDocumentView;
...
class KnowledgeDocumentWebContractTest {
    ...
    @MockitoBean
    private KnowledgeDocumentLifecycleUseCase lifecycle;

    @MockitoBean
    private KnowledgeDocumentImportUseCase imports;

    @MockitoBean
    private KnowledgeIndexJobUseCase indexJobs;
    ...
    void resetSessionsAndUseCases() {
        ...
        reset(queries, scopeResolver, adminQueries, commands, lifecycle, imports, indexJobs);
    }
    ...
    void memberCanReadCompletePublishedDocumentWithoutAdminFields() throws Exception {
        KnowledgeBrowseContext context = new KnowledgeBrowseContext(KnowledgeBrowseContextType.GLOBAL, null, null);
        when(scopeResolver.resolveBrowse(KnowledgeBrowseContextType.GLOBAL, null, null)).thenReturn(context);
        when(queries.get(any())).thenReturn(view());
    ...
    void invalidScopeAndInvisibleDocumentUseStableSafeErrors() throws Exception {
        ...
        when(queries.get(any())).thenThrow(new KnowledgeDocumentNotFoundException());
        mockMvc.perform(get("/api/knowledge-documents/{id}", DOCUMENT_ID)
    ...
    void adminCanListAndReadCompleteKnowledgeMetadata() throws Exception {
        when(adminQueries.list(any())).thenReturn(new PageResult<>(List.of(summary()), 0, 20, 1, 1));
        when(adminQueries.get(DOCUMENT_ID)).thenReturn(view());
    ...
    void adminValidationNotFoundAndConflictErrorsAreStableAndSafe() throws Exception {
        ...
        when(adminQueries.get(DOCUMENT_ID)).thenThrow(new KnowledgeDocumentNotFoundException());
        mockMvc.perform(get("/api/admin/knowledge-documents/{id}", DOCUMENT_ID).cookie(admin))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code").value("DOCUMENT_NOT_FOUND"));
    }
    ...
    void adminLifecycleEndpointsReturnCommittedStateAndReplacementTrace() throws Exception {
        UUID oldId = UUID.fromString("44444444-4444-4444-4444-444444444444");
    ...
    void unknownImportBatchUsesDedicatedNotFoundError() throws Exception {
        UUID batchId = UUID.fromString("55555555-5555-5555-5555-555555555555");
    ...
    void adminSubmitsKnowledgeReindexWithAcceptedSingleFlightView() throws Exception {
        UUID jobId = UUID.fromString("66666666-6666-6666-6666-666666666666");
    ...
    void memberIsForbiddenAndNonKnowledgeJobIsNotFound() throws Exception {
        UUID jobId = UUID.fromString("77777777-7777-7777-7777-777777777777");
        mockMvc.perform(post(KnowledgeIndexJobHttpContract.BASE_PATH).cookie(loginCookie("member")))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.code").value("AUTH_FORBIDDEN"));
        verify(indexJobs, never()).submit();

        when(indexJobs.get(jobId)).thenThrow(new io.github.loredock.knowledge.application.KnowledgeIndexJobNotFoundException());
