...
import io.github.loredock.project.application.*;
import io.github.loredock.project.domain.ProjectStatus;
import io.github.loredock.project.infrastructure.web.AdminProjectController;
import io.github.loredock.project.infrastructure.web.ProjectController;
...
class ProjectWebContractTest {
    ...
    private static final UUID MAIN_ID = UUID.fromString("22222222-2222-2222-2222-222222222222");
    ...
    @MockitoBean
    private ProjectCommandUseCase commands;

    @MockitoBean
    private ProjectQueryUseCase queries;

    @MockitoBean
    private AdminProjectQueryUseCase adminQueries;
    ...
    void resetSessionsAndUseCases() {
        ...
        reset(commands, queries, adminQueries);
    }
    ...
    void ordinaryProjectApiRequiresLoginBeforeQueryDispatch() throws Exception {
        ...
        verify(queries, never()).listEnabledProjects();
    }
    ...
    void memberCannotCreateProjectByCallingAdminApiDirectly() throws Exception {
        ...
        verify(commands, never()).createProject(any());
    }
    ...
    void memberCanListEnabledProjectsAndSelectExplicitBranch() throws Exception {
        when(queries.listEnabledProjects()).thenReturn(List.of(summary()));
        when(queries.getEnabledProject("network-tool", "Feature/Case")).thenReturn(detail("Feature/Case"));
        Cookie member = loginCookie("member");
        ...
        verify(queries).getEnabledProject("network-tool", "Feature/Case");
    }
    ...
    void missingBranchQueryIsPassedAsDefaultSelectionRequest() throws Exception {
        when(queries.getEnabledProject("network-tool", null)).thenReturn(detail("main"));
        ...
        verify(queries).getEnabledProject("network-tool", null);
    }
    ...
    void adminWriteEndpointsMapCompleteContracts() throws Exception {
        ...
        when(commands.createProject(any())).thenReturn(adminDetail);
        when(commands.addBranch(eq(PROJECT_ID), any())).thenReturn(feature);
        when(commands.changeStatus(eq(PROJECT_ID), any())).thenReturn(adminDetail(ProjectStatus.DISABLED));
        ...
        verify(commands).createProject(new CreateProjectCommand(
        ...
        verify(commands).addBranch(PROJECT_ID, new AddBranchCommand("feature/import-export"));
        ...
        verify(commands).changeStatus(PROJECT_ID, new ChangeProjectStatusCommand(ProjectStatus.DISABLED));
    }
    ...
    void adminQueriesReturnDisabledProjectWithAuditFields() throws Exception {
        when(adminQueries.listProjects(ProjectStatus.DISABLED)).thenReturn(List.of(adminSummary(ProjectStatus.DISABLED)));
        when(adminQueries.getProject(PROJECT_ID)).thenReturn(adminDetail(ProjectStatus.DISABLED));
        Cookie admin = loginCookie("admin");
    ...
    void validationNotFoundAndConflictFailuresUseStableApiErrors() throws Exception {
        ...
        when(queries.getEnabledProject("missing-project", null)).thenThrow(new ProjectNotFoundException());
        mockMvc.perform(get("/api/projects/missing-project").cookie(admin))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code").value("PROJECT_NOT_FOUND"));

        when(commands.createProject(any())).thenThrow(new ProjectIdentifierConflictException());
        mockMvc.perform(post("/api/admin/projects")
                        .cookie(admin).contentType(MediaType.APPLICATION_JSON).content(validCreateJson()))
